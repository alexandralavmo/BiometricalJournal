...
data_cpu = data.frame(n = rep(n_scalable, 3), cpu = c(cpu_par, cpu_fim, cpu_time), grp = c(rep("Parameters", length(n_scalable)), rep("Standard errors", length(n_scalable)), rep("Total", length(n_scalable))))
...
data_cpu_k = data.frame(n = rep(k_scalable, 3), cpu = c(cpu_par_k, cpu_fim_k, cpu_time_k), grp = c(rep("Parameters", length(k_scalable)), rep("Standard errors", length(k_scalable)), rep("Total", length(k_scalable))))
