#### To reproduce the violin plots for the relative estimation errors of the true model parameters
...
vioplot1 = ggplot(data = tb1, aes(x = param, y = val)) +
  ...
  scale_x_discrete(labels = c("μ01" = expression(mu["01"]), "μ11" = expression(mu["11"]), "μ21" = expression(mu["21"]), "μa1" = expression(mu["a1"]), "omega_01" = expression(omega["01"]), "omega_11" = expression(omega["11"]), "omega_21" = expression(omega["21"]), "omega_a1" = expression(omega["a1"]), "sigma_b1" = expression(sigma["b1"])))+
...
vioplot2 = ggplot(data = tb2, aes(x = param, y = val)) +
  ...
  scale_x_discrete(labels = c("μ02" = expression(mu["02"]), "μ12" = expression(mu["12"]), "omega_02" = expression(omega["02"]), "omega_12" = expression(omega["12"]), "sigma_a2" = expression(sigma["a2"])))+
  scale_y_continuous(limits = c(-100, 100))+
  theme(axis.text.x = element_text(hjust = 1, size = 8), axis.title = element_text(size = 10), axis.text.y = element_text(size = 8))
...
vioplot3 = ggplot(data = tb3, aes(x = param, y = val)) +
  ...
  scale_y_continuous(limits = c(-100, 100))+
  scale_x_discrete(labels = c("μ03" = expression(mu["03"]), "μ13" = expression(mu["13"]), "omega_03" = expression(omega["03"]), "omega_13" = expression(omega["13"]), "sigma_a3" = expression(sigma["a3"])))+
  theme(axis.text.x = element_text(hjust = 1, size = 8), axis.title = element_text(size = 10), axis.text.y = element_text(size = 8))
...
vioplot4 = ggplot(data = tb4, aes(x = param, y = val)) +
  ...
  scale_x_discrete(labels = c("p1" = expression(p["1"]), "g1" = expression(g["1"]), "alpha11" = expression(alpha["11"]), "alpha12" = expression(alpha["12"]), "alpha13" = expression(alpha["13"])))+
  theme(axis.text.x = element_text(hjust = 1, size = 8), axis.title = element_text(size = 10), axis.text.y = element_text(size = 8))
