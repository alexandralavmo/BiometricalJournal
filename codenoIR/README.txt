...
- application: contains (1) code needed to reproduce figures and table of the application study, 
			(2) datasets corresponding to jittered versions of the true patient datas, and
			(3) model and mlxtran files used in Monolix
			Therefore, tables and figures might slightly differ from the manuscript because the true data cannot be shared.
- simulation: contains  (1) the code needed to reproduce figures and tables of the simulation study
			(2) simulated datasets that can be used to guarantee the reproductibilty of the results, and
			(3) model and mlxtran files used in Monolix
...
subfolder "intermediate_results": is devoted to store intermediate results, and remains empty until computation have not been started 
...
By default, these scripts compute all needed intermediate results from scratch. Beware that, since intermediate results are not provided in this 
version of the code, it will likely raise an error. Another version of the code, in which intermediate results are provided, is available at
https://github.com/alexandralavmo/BiometricalJournal/tree/main/codeIR
